...
### 2.4 Ping/Pongプロトコル実装詳細
ヘルス監視は以下の手順で実装:
1. Orchestrator: `ping_interval` (default: 5s) ごとに `ControlMessage(type="PING", payload={"ping_id": uuid4()})` を各Worker のcontrol_pipeに送信
2. Worker: PINGメッセージ受信時、`StatusUpdate(camera_id=camera_id, status="RUNNING", ping_response=ping_id)` を返送
3. Orchestrator: `ping_timeout` (default: 10s) 以内にping_responseが返らない場合、ping_loss_count をインクリメント
4. `ping_loss_count >= max_ping_failures` (default: 3) に達したWorkerは status=DOWN に移行、再起動ポリシー適用

技術補足:
- Ping送信はメインスレッドでなくOrchestratorのhealth_monitor_threadで処理
- StatusUpdate.ping_responseフィールドがNoneでない場合のみpong応答として処理
- Workerプロセス内でのping処理は推論ループをブロックしないよう非同期チェック実装
