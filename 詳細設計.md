...
| パス | 役割 | 備考 |
|------|------|------|
| services/camera.py | カメラキャプチャ（複数カメラ対応）、フレーム投入 | CameraService | FR-001, FR-002 |
| services/inference.py | 推論ワーカー(並列/マルチプロセス) | InferenceService | FR-003, FR-004 |
| services/openvino_adapter.py | OpenVINO 初期化 & 推論ラッパ | OpenVINOAdapter | FR-003 |
| services/aggregator.py | 最新推論イベント集約 | ResultAggregator | FR-003, FR-005 |
| services/recording.py | 映像 & JSONL メタ記録 | RecordingService | FR-006 |
| services/config.py | 設定読込/バリデーション | ConfigService | FR-009 |
| services/metrics.py | メトリクス収集 & JSON 出力 | MetricsService | NFR-性能監視 |
| services/health.py | ヘルス/遅延/ドロップ監視 | HealthWatcher | FR-011, FR-012 |
| services/offline_guard.py | 外部通信抑止ガード | OfflineGuard | セキュリティ |
| ProgramName/models/types.py | データクラス/型定義 | dataclass+typing |
| ProgramName/common/base_exception.py | 例外基底 | ProjectError 階層 |
| ProgramName/common/logging_setup.py | ログ初期化 | JSON化拡張余地 |
| ProgramName/resources/ApplicationConfig.xml | 設定 | 環境変数不使用 |
| ProgramName/results/ | 録画/メトリクス出力 | ローテ対象 |
| ProgramName/tests/unit/ | 単体テスト | pytest |
| ProgramName/tests/integration/ | 結合テスト | シナリオ |
