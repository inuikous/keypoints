...
バージョン: v0.3  (更新日: YYYY-MM-DD)  ※v0.2→v0.3 変更要約: ExitNotice へ終了通知統一 / 統合Worker注記 / ResultDispatcherThread 明確化 / 再起動&ヘルス閾値追加 / ドロップアルゴリズム具体化 / Export仕様詳細化 / timestamp & monotonic 方針 / pickle リスク明示 / InferenceError 追加 / ログ & メトリクス閾値設定。
...
### 2.3 プロセス/IPC モデル
- メイン: Orchestrator + Aggregator + GUI + LogListenerThread + MetricsThread
- N 子プロセス: CaptureInferenceWorker (初期統合) — 将来分離( Capture / Inference )拡張余地。
- IPC 要素:
  - `result_queue`: `multiprocessing.Queue[ResultRecord | StatsMessage]` (容量上限 configurable, デフォ 1024)
  - `control_pipes`: 親⇄各子 (双方向 `multiprocessing.Pipe`) で制御 / HealthPing / StatusUpdate 伝達
  - `log_queue`: `multiprocessing.Queue[LogRecord]` (QueueHandler 経由で集約)
  - (将来) `frame_shm` 共有メモリ (最適化時)。
- 終了シーケンス(標準):
  1. Orchestrator.stop() 呼出
  2. 親→各子へ ControlMessage(type=STOP)
  3. 子: ループ break → 残キュー flush (ResultRecord) → ExitNotice(code,reason) 送信 → 正常終了
  4. 親: join(timeout) → 未終了は terminate → ログ出力 (WORKER_FORCE_TERMINATE)
  5. 親: result_queue に SENTINEL 投入 → ResultDispatcherThread 終了 → Aggregator 受信完了
  6. LogListenerThread: `None` (sentinel) 受領で flush & close

### 2.4 IPC メッセージ種別 (概要)
| 種別 | 方向 | フィールド例 | 用途 |
|------|------|--------------|------|
| ControlMessage | 親→子 | type: START/STOP/RELOAD | 開始/停止/設定再読込(将来) |
| StatusUpdate | 子→親 | camera_id, status: RUNNING/RETRYING/DOWN | 状態監視/GUI反映 |
| HealthPing / Pong | 親↔子 | ts_send, ts_recv | ヘルス/遅延測定 |
| StatsMessage | 子→親 | camera_id, fps_window, avg_latency | Aggregator 統計合成 |
| ExitNotice | 子→親 | code, reason | 異常/正常終了判定 (ControlMessage で EXITING は使用しない) |
...
| metrics thread | Aggregator 統計を一定周期 (1s) で集計/差分ログ | `threading.Thread` |
...
| Aggregator | 結果集約/リングバッファ/統計融合/クエリ | ResultRecord/StatsMessage | ExportData/ViewModel | バッファ溢れ→古い順破棄 WARN |
...
| MetricsThread | 定周期集計/Publish | Aggregator | StatsMessage(Log) | 計測失敗→WARN |
...
build_status_view() -> DashboardState
...
| ControlMessage | type / payload(dict) | STOP/RELOAD/PING |
| StatusUpdate | camera_id / status / attempts / last_error? | GUI 表示用 |
| StatsMessage | camera_id / fps / avg_latency / drop_rate | 集計融合 |
...
- バックプレッシャ: `result_queue` が満杯の場合は「put_nowait → Full なら get_nowait で1件破棄 → 再 put」。破棄件数を drop counter として StatsMessage に反映。
...
- ヘルス監視: ping_interval=5s, ping_timeout=10s, 連続 ping_loss=3 で StatusUpdate(status=DOWN,last_error=ping_timeout)。
...
```
構造化キー: ts / level / event / camera / pid / process_name / msg / exc / latency_ms / fps / drop_rate。
